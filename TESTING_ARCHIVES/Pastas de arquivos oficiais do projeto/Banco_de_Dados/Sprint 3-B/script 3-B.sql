...
FROM leitura  AS l 
	JOIN sensor AS s ON fkSensor = idSensor
		WHERE 20 < (SELECT TRUNCATE(AVG(leituraUmidadeSolo), 1) AS 'Leitura média da umidade do solo' FROM leitura)
...
FROM leitura  AS l 
    ...
    select s.idSensor from sensor s join plantacao p on p.idPlantacao = s.fkPlantacao join empresa e on e.idEmpresa = p.fkEmpresa where e.idEmpresa = 1;
